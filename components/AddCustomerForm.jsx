"use client";

import { useState, useRef } from "react";
import { Lock, Plus, Loader2, CheckCircle2 } from "lucide-react";
import { submitCustomer } from "@/lib/actions";

export default function AddCustomerForm() {
  const [unlocked, setUnlocked] = useState(false);
  const [pin, setPin] = useState("");
  const [pinError, setPinError] = useState("");
  const [pinChecking, setPinChecking] = useState(false);
  const [status, setStatus] = useState("idle");
  const [error, setError] = useState("");
  const formRef = useRef(null);

  const checkPin = async (e) => {
    e.preventDefault();
    setPinChecking(true);
    setPinError("");
    try {
      const res = await fetch("/api/auth/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pin }),
      });
      if (res.ok) {
        setUnlocked(true);
      } else {
        const data = await res.json();
        setPinError(data.error || "Incorrect PIN.");
      }
    } catch {
      setPinError("Could not verify — try again.");
    } finally {
      setPinChecking(false);
      setPin("");
    }
  };

  const handleSubmit = async (formData) => {
    setError("");
    setStatus("saving");
    const res = await submitCustomer(formData);
    if (res.ok) {
      setStatus("sent");
      formRef.current?.reset();
    } else {
      setStatus("idle");
      setError(res.error);
    }
  };

  if (!unlocked) {
    return (
      <div className="border border-slate-200 bg-slate-50 px-5 py-5 max-w-xs">
        <div className="flex items-center gap-2 mb-3 text-slate-500">
          <Lock size={14} /> <span className="font-mono text-[11px] uppercase tracking-wider">Admin only</span>
        </div>
        <form onSubmit={checkPin} className="flex gap-2">
          <input
            type="password" autoComplete="current-password"
            value={pin} onChange={(e) => setPin(e.target.value.trim())}
            placeholder="PIN"
            className="flex-1 bg-white border border-slate-300 focus:border-amber-500 outline-none px-3 py-2 text-sm text-slate-700 font-mono"
          />
          <button disabled={pinChecking} className="bg-slate-900 disabled:bg-slate-400 text-white font-mono uppercase text-xs px-4">
            {pinChecking ? <Loader2 size={13} className="animate-spin" /> : "Unlock"}
          </button>
        </form>
        {pinError && <p className="text-red-500 text-xs font-mono mt-2">{pinError}</p>}
      </div>
    );
  }

  return (
    <div className="border border-slate-200 bg-slate-50 px-5 py-5 max-w-md">
      <h3 className="text-slate-900 font-semibold text-sm mb-3">Add a Customer</h3>
      {status === "sent" ? (
        <div className="flex items-center gap-2 text-emerald-600 text-sm font-medium">
          <CheckCircle2 size={16} /> Customer added.
          <button onClick={() => setStatus("idle")} className="font-mono text-xs uppercase text-amber-600 ml-2">Add another</button>
        </div>
      ) : (
        <form ref={formRef} action={handleSubmit} className="space-y-2">
          <input name="name" placeholder="Customer / company name" className="w-full bg-white border border-slate-300 focus:border-amber-500 outline-none px-3 py-2 text-sm text-slate-700" />
          <input name="sector" placeholder="Sector (e.g. Banking & Finance)" className="w-full bg-white border border-slate-300 focus:border-amber-500 outline-none px-3 py-2 text-sm text-slate-700" />
          <input name="note" placeholder="Short project note" className="w-full bg-white border border-slate-300 focus:border-amber-500 outline-none px-3 py-2 text-sm text-slate-700" />
          {error && <p className="text-red-500 text-xs font-mono">{error}</p>}
          <button disabled={status === "saving"} className="bg-amber-500 disabled:bg-slate-300 hover:bg-amber-400 text-slate-950 font-mono uppercase text-xs tracking-wider px-4 py-2.5 flex items-center gap-1.5">
            {status === "saving" ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />} Add Customer
          </button>
        </form>
      )}
    </div>
  );
}
